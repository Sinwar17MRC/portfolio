...
export const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
      scrolled ? 'bg-slate-900/90 backdrop-blur-sm border-b border-amber-900/20' : 'bg-transparent'
    }`}>
          ...
          <div className="hidden md:flex space-x-8">
            {navItems.map((item) => (
              <a
                ...
                className={`text-slate-300 hover:text-amber-300 transition-all duration-300 relative group ${
          ...
          <div className="md:hidden bg-slate-800/95 backdrop-blur-sm rounded-lg mt-2 p-4 border border-amber-900/20">
            {navItems.map((item) => (
              <a
                ...
                href={item.href}
                onClick={(e) => handleNavClick(e, item.href)}
                className={`block py-2 text-slate-300 hover:text-amber-300 transition-colors duration-200 ${
                  activeSection === item.id ? 'text-amber-400' : ''
                }`}
              >
                {item.label}
